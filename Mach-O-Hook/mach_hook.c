...
{
    void const *library_address;  //动态库基地
    char const *string_table;  //str表
    struct nlist const *symbol_table;  // 符号表
    uint32_t const *indirect_table;  //间接符号表
    uint32_t undefined_symbols_count;  //符号表中未定义的符号数
    uint32_t undefined_symbols_index;  //未定义符号在符号中的位置  table
